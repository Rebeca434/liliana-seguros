      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-[1.2fr_0.8fr] gap-8 md:gap-16 items-center relative z-10 w-full">
        ...
        >
          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-serif text-navy leading-[1.1] mb-6 md:mb-8 font-bold">
            Tu tranquilidad,<br />
            <span className="text-gold italic">nuestra prioridad.</span>
          </h1>
        ...
        >
          <div className="relative group overflow-hidden rounded-2xl">
            <img
              src="https://i.imgur.com/iZmxOjT.jpeg"
...
const DetailsItem = ({ term, desc }: { term: string; desc: string }) => {
  const [open, setOpen] = useState(false);
  return (
    <div
      className="flex flex-col p-4 rounded-lg bg-slate-50 border border-slate-100 border-l-4 border-l-gold cursor-pointer transition-all hover:bg-slate-100"
      onClick={() => setOpen(!open)}
    >
      <div className="flex items-start gap-4">
        <div className="mt-1 text-navy shrink-0">
          <ChevronDown size={20} className={`transition-transform duration-300 ${open ? "rotate-180" : ""}`} />
        </div>
        <span className="font-bold text-navy text-sm leading-tight">{term}</span>
      </div>
      <AnimatePresence>
        {open && (
          <motion.div
          ...
          >
            <p className="text-slate-600 text-sm ml-9 leading-relaxed">{desc}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
            ...
            >
              ...
              <div className="max-w-2xl">
                ...
                <div className="grid sm:grid-cols-2 gap-6 mb-10">
                  {[
                    { term: "Deducible", desc: "Es la cantidad inicial fija que debes cubrir antes de que el seguro comience a pagar." },
                    { term: "Coaseguro", desc: "Es un porcentaje del gasto total que asumes junto con la aseguradora una vez cubierto el deducible." },
                    { term: "Nivel hospitalario", desc: "Determina a qué red de hospitales tienes acceso, desde básicos hasta los de máxima especialidad." },
                    { term: "Suma asegurada", desc: "Es el límite máximo de dinero que la aseguradora pagará por cada enfermedad o accidente." },
                    { term: "Honorarios quirúrgicos", desc: "Es el tabulador que define cuánto se le paga a los médicos, cirujanos y especialistas por sus servicios." },
                    { term: "Exención de deducible por accidente o emergencia en el extranjero", desc: "Beneficio donde no pagas deducible si sufres un accidente o necesitas atención médica de emergencia estando fuera del país." }
                  ].map((item, i) => (
                    <DetailsItem key={i} term={item.term} desc={item.desc} />
                  ))}
                </div>
...
const ProcessSection = () => {
  const steps = [
    { title: "Asesoría personalizada sin costo", icon: UserCheck, desc: "Platicamos sobre tus necesidades y las de tu familia para entender qué tipo de cobertura es la ideal para ti." },
    { title: "Buscamos la mejor opción entre las mejores aseguradoras", icon: Search, desc: "Cotizamos y comparamos entre las principales aseguradoras del país para encontrar el mejor plan para tu perfil." },
    { title: "Encontramos la cobertura adecuada", icon: ShieldCheck, desc: "Te presentamos opciones claras y transparentes, sin letra pequeña, para que tomes la mejor decisión." },
    { title: "Contrata sin complicaciones", icon: FileCheck, desc: "El proceso de contratación es sencillo y lo acompañamos paso a paso contigo." },
    { title: "Vive tranquilo y protegido", icon: HeartPulse, desc: "Ante cualquier siniestro, estamos contigo para orientarte y hacer válida tu cobertura." },
  ];
          ...
          {steps.map((step, i) => (
            <React.Fragment key={i}>
              <div className="flex flex-col items-center gap-4 text-center group relative">
                ...
                {i < steps.length - 1 && (
                  <div className="hidden md:block absolute top-8 left-[60%] w-full h-[1px] bg-gold/30 z-0" />
                )}
                <div className="w-16 h-16 rounded-full bg-navy border-2 border-gold flex items-center justify-center text-gold relative z-10 group-hover:bg-gold group-hover:text-navy transition-all duration-300 shrink-0">
                  <step.icon size={26} />
                </div>
                <p className="text-white font-bold text-[11px] uppercase tracking-wider leading-relaxed">
                  {step.title}
                </p>
                <p className="text-slate-400 text-[11px] leading-relaxed hidden lg:block">
                  {step.desc}
                </p>
              </div>
            </React.Fragment>
...
const DeductibilidadSection = () => (
  <section className="py-24 bg-gradient-to-br from-navy to-blue-900 relative overflow-hidden">
    ...
    <div className="max-w-7xl mx-auto px-6 relative z-10">
      <div className="grid md:grid-cols-2 gap-16 items-center">
        <div>
          <span className="text-gold font-bold uppercase tracking-widest text-[10px] mb-4 block">Beneficio Fiscal</span>
          <h2 className="text-3xl md:text-5xl font-serif font-bold text-white mb-6 leading-tight">
            Tu seguro es <span className="text-gold italic">deducible de impuestos</span>
          </h2>
          <p className="text-slate-300 text-lg leading-relaxed mb-8">
            El Seguro de Gastos Médicos Mayores es una <strong className="text-white">deducción personal autorizada por el SAT</strong>. Esto significa que puedes deducir las primas que pagues en tu declaración anual de impuestos, reduciendo tu base gravable.
          </p>
          <div className="space-y-4">
            {[
              { icon: CheckCircle2, text: "Las primas pagadas son deducibles en tu declaración anual" },
              { icon: CheckCircle2, text: "Aplica para seguros contratados a tu nombre y de tu familia directa" },
              { icon: CheckCircle2, text: "Es un beneficio adicional que reduce tu carga fiscal" },
            ].map((item, i) => (
        ...
        <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-10">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <p className="text-white font-bold text-lg">¿Cuánto puedes deducir?</p>
              <p className="text-slate-400 text-sm">Límite establecido por el SAT</p>
            </div>
          </div>
          <div className="space-y-6">
            <div className="border-b border-white/10 pb-6">
              <p className="text-slate-400 text-xs uppercase tracking-widest mb-2">¿Qué es deducible?</p>
              <p className="text-white font-bold">Las primas del seguro médico que pagas anualmente por ti, tu cónyuge, hijos y padres.</p>
            </div>
            <div className="border-b border-white/10 pb-6">
              <p className="text-slate-400 text-xs uppercase tracking-widest mb-2">¿Cuándo aplica?</p>
              <p className="text-white font-bold">Al presentar tu Declaración Anual ante el SAT, en el apartado de deducciones personales.</p>
            </div>
            <div>
              <p className="text-slate-400 text-xs uppercase tracking-widest mb-2">¿Qué necesitas?</p>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-6 group border-b border-light-blue pb-6">
                ...
                <div>
                  ...
                  <p className="text-navy text-base sm:text-lg font-bold tracking-tight">lsaenzpardop@inbursaasesores.com</p>
                </div>
              </div>
        ...
        <div className="mt-12 pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div className="text-left">
              ...
              <a href="/cedula-profesional.pdf" target="_blank" rel="noopener noreferrer" className="text-xs font-bold hover:text-gold transition-colors flex items-center gap-1">Ver Cédula <ExternalLink size={10} /></a>
            </div>
          </div>
    ...
    <div className="font-sans text-slate-900 bg-white selection:bg-gold/30">
      ...
      <main>
        ...
        <section id="servicios" className="bg-slate-50">
          {/* Banner con foto de familia */}
          <div className="relative h-64 md:h-80 overflow-hidden">
            <img
              src="https://images.unsplash.com/photo-1531983412531-1f49a365ffed?w=1600&auto=format&fit=crop&q=80"
              alt="Familia con seguro"
              className="w-full h-full object-cover object-center"
            />
            <div className="absolute inset-0 bg-navy/60 flex flex-col items-center justify-center text-center px-6">
              <span className="text-gold font-bold uppercase tracking-widest text-[10px] mb-3">Soluciones completas</span>
              <h2 className="text-3xl md:text-5xl font-serif font-bold text-white leading-tight">
                Portafolio Integral
              </h2>
              <div className="h-1 w-16 bg-gold mx-auto mt-4" />
              <p className="text-slate-200 text-sm md:text-base mt-4 max-w-xl leading-relaxed">
                Aunque nos especializamos en salud, ofrecemos soluciones completas para todas las etapas de tu vida.
              </p>
            </div>
          </div>
